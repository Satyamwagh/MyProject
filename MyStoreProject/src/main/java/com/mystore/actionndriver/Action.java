package com.mystore.actionndriver;
import java.io.File;
...
public class Action extends BaseClass {
	...
	public static void click(WebDriver driver, WebElement locaterName) {
		Actions act = new Actions(driver);
	//	new WebDriverWait(driver, 15).until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//iframe"))); 
		act.moveToElement(locaterName).build().perform();
		locaterName.click();
	...
	public static boolean findElement(WebDriver Idriver, WebElement ele) {
		...
		try {
			...
			if (flag) {
				System.out.println("Successfully find element at");
			} else {
	...
	public static boolean selectByVisibaleText(String visibletext, WebElement ele) {
		boolean flag = false;
		try {
	...
	public static boolean mouseHoverByJavaScript(WebElement locater) {
		boolean flag = false;
		try {
			WebElement mo = locater;
			String javaScript = "var evObj = document.createEvent('MauseEvents')"
